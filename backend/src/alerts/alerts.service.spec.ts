...
describe('AlertsService', () => {
        ...
        AlertsService,
        {
          ...
          useValue: {
            alerts: {
              ...
              findMany: jest.fn(),
              update: jest.fn(),
              aggregate: jest.fn().mockReturnValue({ _max: { id: 0n } }),
            },
            event: {
              findUnique: jest.fn(),
            },
  ...
  describe('create', () => {
    it('should create alert with valid event', async () => {
      (prisma.event.findUnique as jest.Mock).mockResolvedValue({ id: 1n });
  ...
  describe('assignAlertToSelf', () => {
    it('should assign alert', async () => {
      ...
      (prisma.users.findUnique as jest.Mock).mockResolvedValue({
        staff_details: { id: 1n, organization_id: 'org1' },
      });
      (prisma.alerts.findUnique as jest.Mock).mockResolvedValue({
        id: 1n,
        event_id: 1n,
        staff_id: null,
        event: { organization_id: 'org1' },
      });
      ...
      const result = await service.assignAlertToSelf(user as any, '1');
      expect(result.staff_id).toBe(1n);
    });

    it('should throw if not authenticated', async () => {
      await expect(service.assignAlertToSelf(undefined, '1')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  });
  ...
  describe('findByOrganizationEvent', () => {
    it('should return alerts', async () => {
      const user = { id: 'user1' };
      (prisma.users.findUnique as jest.Mock).mockResolvedValue({
        staff_details: { organization_id: 'org1' },
      });
      ...
      const result = await service.findByOrganizationEvent(user as any, '1');
